...
class SequenceGenerator:
    ...
    def generate_greedy(
        ...
        for _ in range(self.max_length - cur_len):
            ...
            logits = self.score_fn(seq)          # (B, vocab)
            ...
            # 5) Scores corresponding to chosen tokens
            next_token_scores = log_probs.gather(
                1, next_tokens.unsqueeze(1)
            ).squeeze(1)                                    # (B,)

            # 6) Update scores only for unfinished sequences
            scores = scores + torch.where(
                finished,
                torch.zeros_like(next_token_scores),
                next_token_scores,
            )

            # 7) Append chosen tokens
            seq = torch.cat([seq, next_tokens.unsqueeze(1)], dim=1)  # (B, t+1)

            # 8) Update finished flags when EOS is generated
            eos = getattr(self, "eos_token", None)
            if eos is None:
                eos = getattr(self, "eos_id", None)
            if eos is None:
                eos = getattr(self, "eos_token_id", None)
            if eos is not None:
                newly_finished = next_tokens.eq(eos)
